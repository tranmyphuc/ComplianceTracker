...
export function AIMascot({ 
  ...
  return (
    <div className={`relative flex flex-col items-center ${className}`}>
      <motion.div 
        className={`relative ${sizeClasses[size]} bg-purple-500 rounded-full overflow-hidden`}
      ...
      >
        ...
        <div className={`absolute ${size === "sm" ? "w-1.5 h-3" : size === "md" ? "w-2 h-5" : "w-3 h-8"} bg-purple-600 rounded-l-full -left-0.5 top-1/2 transform -translate-y-1/2`}></div>
        <div className={`absolute ${size === "sm" ? "w-1.5 h-3" : size === "md" ? "w-2 h-5" : "w-3 h-8"} bg-purple-600 rounded-r-full -right-0.5 top-1/2 transform -translate-y-1/2`}></div>
      </motion.div>

...
import { euAiActArticles } from './shared/schema.ts';
...
async function seedEuAiActArticles() {
  ...
  try {
    ...
    const articleData = [
      ...
      },
      {
        ...
        keyPoints: JSON.stringify([
          ...
          "Humans must be able to monitor system operation",
          "Users must be aware of automation bias",
          "Users must be able to override system decisions",
          "Stop button or similar intervention mechanism must be available"
        ]),
        version: "1.0",
        lastUpdated: new Date(),
        isLatest: true,
        exampleSummary: "This article requires human oversight measures for high-risk AI systems to prevent risks and enable humans to monitor, understand, and override AI systems when necessary.",
    ...
    for (const article of articleData) {
      ...
      console.log(`Added article: ${article.articleId}`);
    }

    console.log('EU AI Act articles seeded successfully!');
  } catch (error) {

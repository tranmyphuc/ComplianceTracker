...
// Helper function to create interactive quiz questions
function createInteractiveQuiz(moduleId: string, slideIndex: number): any {
  // Module 1 quizzes
  if (moduleId === '1') {
    // First quiz for EU AI Act intro module
    if (slideIndex === 2) {
      return {
        question: "Which of the following is not one of the main risk categories in the EU AI Act?",
        options: [
          "Unacceptable risk",
          "High risk",
          "Moderate risk", // This is the incorrect one
          "Limited risk"
        ],
        correctAnswer: 2,
        explanation: "The EU AI Act defines four risk categories: unacceptable risk, high risk, limited risk, and minimal risk. 'Moderate risk' is not one of the official categories."
      };
    }
    // Second quiz
    if (slideIndex === 5) {
      return {
        question: "When will prohibited AI practices (Article 5) come into effect?",
        options: [
          "Immediately after publication",
          "6 months after entry into force",
          "12 months after entry into force",
        ...
        correctAnswer: 0,
        explanation: "Prohibited AI practices under Article 5 come into effect immediately after publication, reflecting their critical importance for safety and fundamental rights."
      };
    }
  }
  
  // Module 2 quizzes (Risk Classification)
  else if (moduleId === '2') {
    if (slideIndex === 2) {
      return {
        question: "Which of these AI systems would NOT be classified as high-risk?",
        options: [
          "AI system for critical infrastructure management",
          "AI system for recruitment and worker evaluation",
          "AI customer service chatbot for a retail store", // Correct answer
          "AI system for determining access to essential public services"
        ],
        correctAnswer: 2,
        explanation: "A customer service chatbot for a retail store would typically fall under 'limited risk' category, requiring only transparency obligations. The other options are explicitly listed as high-risk in Annex III."
      };
    }
  }
  
  // Module 3 quizzes (Technical Requirements)
  else if (moduleId === '3') {
    if (slideIndex === 2) {
      return {
        question: "Which technical requirement applies to high-risk AI systems?",
        options: [
          "Open-source publication of the full codebase",
          "Risk management system and data governance",
          "Mandatory 24/7 human monitoring",
          "Quantum encryption of all data processes"
        ],
        correctAnswer: 1,
        explanation: "High-risk AI systems require a risk management system and data governance measures. The other options are not mandated by the EU AI Act."
      };
    }
  }
  
  // Default quiz for any module/slide
  return {
    question: "Which EU institution was the primary drafter of the EU AI Act?",
    options: [
    ...
    correctAnswer: 2,
    explanation: "The European Commission developed and proposed the EU AI Act as part of its regulatory approach to artificial intelligence."
  };
}

// Helper function to create role-specific relevance information
function createRoleRelevance(moduleId: string, slideTitle: string, userRole: string): any {
  // Define default role relevance
  const defaultRelevance = {
    decision_maker: "This content provides strategic insight into compliance requirements.",
    developer: "This information helps you understand technical implementation needs.",
    operator: "This knowledge supports effective AI system management.",
    legal: "These details are essential for ensuring legal compliance.",
    general: "This information is relevant for all roles within the organization."
  };
  
  // Module-specific customizations
  if (moduleId === '1' && slideTitle.includes("Introduction")) {
    return {
      decision_maker: "Understanding the EU AI Act scope is crucial for strategic planning and resource allocation.",
      developer: "Knowing the regulatory framework helps you design compliant systems from the start.",
      operator: "This overview helps you understand how your operational procedures may need to change.",
      legal: "This foundation is essential for interpreting specific compliance requirements.",
      general: "This overview provides context for all organizational AI governance."
    };
  }
  
  if (moduleId === '2' && slideTitle.includes("Risk")) {
    return {
      decision_maker: "Risk classification directly impacts business strategy and required investments.",
      developer: "Understanding risk categories helps you implement appropriate technical safeguards.",
      operator: "Risk levels determine the operational controls you'll need to implement.",
      legal: "Risk classification is the foundation for determining applicable legal requirements.",
      general: "Understanding risk categories helps everyone contribute to compliance."
    };
  }
...
// Helper function to create role-specific tips
function createRoleTips(moduleId: string, slideTitle: string, userRole: string): any {
  // Create empty object for all roles
  ...
  if (moduleId === '1') {
    return {
      decision_maker: "Consider creating a cross-functional EU AI Act compliance team with representatives from legal, technical, and business units.",
      developer: "Start documenting your AI development processes now to prepare for technical documentation requirements.",
      operator: "Begin identifying which of your current systems might fall under high-risk categories.",
      legal: "Map the Act's implementation timeline against your product development roadmap to prioritize compliance work.",
      general: "Familiarize yourself with basic AI terminology to better understand compliance requirements."
    };
  }
  ...
  if (moduleId === '2') {
    return {
      decision_maker: "Allocate resources based on risk categories - high-risk systems will require significantly more compliance investment.",
      developer: "Create a risk assessment template to use during the design phase of new AI systems.",
      operator: "Develop monitoring procedures that align with the risk level of each AI system you manage.",
      legal: "Create a risk classification checklist to systematically evaluate new and existing AI systems.",
      general: "Learn to recognize features that might elevate an AI system's risk classification."
    };
  }
  
  // Only provide tips for specific roles on specific slides to avoid overwhelming users
  if (userRole === 'decision_maker' && slideTitle.includes("Requirements")) {
    return {
      ...emptyTips,
      decision_maker: "When budgeting for AI projects, factor in the compliance costs based on risk category."
    };
  }
  
  if (userRole === 'developer' && slideTitle.includes("Technical")) {
    return {
      ...emptyTips,
      developer: "Start implementing data quality checks and documentation in your current development workflow."
    };
  }
  
  if ((userRole === 'legal' && slideTitle.includes("Legal")) || slideTitle.includes("Compliance")) {
    return {
      ...emptyTips,
      legal: "Create compliance checklists specific to each risk category for more efficient assessments."
    };
  }
  
  // Return empty tips if no specific tips apply
  return emptyTips;
}

// Helper function to build slides from content
function buildSlides(content: any, userRole: string = 'general', moduleId: string = '') {
  // If content already has slides, use them
  if (content.slides) {
    // For each slide, add interactive quiz elements and role-specific content if not present
    return content.slides.map((slide: any, index: number) => {
      // Create a new slide object to avoid mutating the original
      const enhancedSlide: any = { ...slide };
      
      // Add interactive quiz if not present (to every 3rd slide as an example)
      if (index % 3 === 2 && !slide.quiz) {
        enhancedSlide.quiz = createInteractiveQuiz(moduleId, index);
      }
      
      // Add role-specific relevance information if not present
      if (!slide.pathRelevance) {
        enhancedSlide.pathRelevance = createRoleRelevance(moduleId, slide.title, userRole);
      }
      
      // Add role-specific tips if not present
      if (!slide.roleTips) {
        enhancedSlide.roleTips = createRoleTips(moduleId, slide.title, userRole);
      }
  ...
  if (content.sections && Array.isArray(content.sections)) {
    content.sections.forEach((section: any, index: number) => {
      ...
      const slide: any = {
        ...
        pathRelevance: createRoleRelevance(moduleId, section.title, userRole),
        roleTips: createRoleTips(moduleId, section.title, userRole)
      };
      
      // Add quiz to every 3rd slide
      if (index % 3 === 2) {
        slide.quiz = createInteractiveQuiz(moduleId, index);
      }
...
function buildExercises(content: any, moduleId: string, userRole: string = 'general'): any[] {
  if (content.exercises) {
    // If the content has role-specific exercises, use those
    if (content.roleExercises && content.roleExercises[userRole]) {
      return content.roleExercises[userRole];
    }
    return content.exercises;
  }

  // Generate default exercises based on module ID and user role
  const defaultExercises = [
    ...
    },
    {
      ...
      description: "<p>Apply your knowledge to analyze real-world compliance scenarios with feedback.</p>",
      tasks: [
        "Review the provided case study scenario",
        "Identify the key compliance requirements applicable",
        "Outline recommended compliance actions",
        "Compare your analysis with the expert solution"
      ],
      roleRelevance: {
        decision_maker: "High",
        developer: "Medium",
      ...
      interactive: true,
      scenario: "A healthcare provider is implementing a diagnostic AI system that analyzes medical images.",
      questions: [
        {
          question: "Which risk category would this system likely fall under?",
          options: ["Minimal risk", "Limited risk", "High risk", "Unacceptable risk"],
          correctAnswer: 2,
          explanation: "Medical diagnostic systems are explicitly listed in Annex III as high-risk AI systems."
        },
        {
          question: "What key documentation requirements apply?",
          options: [
            "Only privacy policy documentation",
            "Technical documentation, risk assessment, and post-market monitoring plan",
            "Simply notifying patients of AI use",
            "No special documentation as healthcare is exempt"
          ],
          correctAnswer: 1,
          explanation: "High-risk AI systems require comprehensive technical documentation, risk assessments, and ongoing monitoring."
        }
      ]
    }
  ];
